  ...
  <div class="bg-white p-4 rounded-lg shadow-md w-full min-w-0 max-w-[500px]">
    ...
    <div class="grid grid-cols-6 gap-2 text-sm mb-2">
      <template v-for="label in ['1', '2', '3']" :key="label">
        ...
        <input
          v-model.number="form[`length${label}`]"
          type="number"
          class="p-1 border rounded-md"
        />
        <input
          v-model.number="form[`depth${label}`]"
          type="number"
          class="p-1 border rounded-md"
        />
        <input
          v-model.number="form[`frontEdge${label}`]"
          type="number"
          class="p-1 border rounded-md"
        />
        <input
          v-model.number="form[`backWall${label}`]"
          type="number"
          class="p-1 border rounded-md"
        />
        <input
          v-model.number="form[`wrapBack${label}`]"
          type="number"
          class="p-1 border rounded-md"
        />
      </template>
...
export default {
  ...
  props: {
    ...
    hondimode: { tyep: Boolean, default: false },
  },
  setup(props, { emit }) {
    ...
    const calculate = () => {
      ...
      const frontEdgeLength = f.oneOpen
        ? f.duOpen
          ? f.length1 + f.length2 + f.length3 + f.depth1 + f.depth3
          : f.length1 + f.length2 + f.length3 + f.depth1
        : f.duOpen
        ? f.length1 + f.length2 + f.length3 + f.depth2
      ...
      const totalArea = Math.round(
        side1.area + side2.area + side3.area - deductArea
      );
      const steps = `A：${side1.calcSteps}\nB：${side2.calcSteps}\nC：${side3.calcSteps}\n${side1.cmValue}+${side2.cmValue}+${side3.cmValue} - ${deduct1} - ${deduct2} = ${total} 公分`;
      const subtotal2 =
        (side1.area + side2.area + side3.area - deductArea) * props.sepPrice;
      ...
      emit("update-result", {
        ...
        detail: {
          ...
          side3: {
            length: f.length3,
            depth: f.depth3,
            frontEdge: f.frontEdge3,
            backWall: f.backWall3,
            wrapBack: f.wrapBack3,
            cmValue: side3.cmValue,
          },
        },
      });
    };
